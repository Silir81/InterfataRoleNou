...
title_label = ctk.CTkLabel(title_bar, text="Smecherie by SRV - V1.1 - 2024.04.01", fg_color='red', text_color='black',
                           font=('Calibri', 20))
...
def update_treeview_function():
    for item in tree.get_children():
        tree.delete(item)
    for index, row in df.iterrows():
        tree.insert("", tk.END, values=(row['Tambur'], row['KG / Rola'], row['Nr. Intern Rola']))
...
def refresh_excel_file():
    ...
    if file_path:
        try:
            ...
            update_treeview_function()
            print("Excel file refreshed")
        except Exception as e:
            msgbox.showerror("Error", f"Failed to refresh Excel file: {e}")
    # Schedule this function to run again after 3600000 milliseconds (1 minute)
    root.after(3600000, refresh_excel_file)
